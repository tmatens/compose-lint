...
```bash
curl -fsSLO https://raw.githubusercontent.com/tmatens/compose-lint/v0.27.0/requirements.lock
pip install --require-hashes -r requirements.lock   # dependencies, hash-pinned
pip install --no-deps compose-lint==0.27.0          # the tool, version-pinned
```

**Docker** — [composelint/compose-lint](https://hub.docker.com/r/composelint/compose-lint)

```bash
docker run --rm -v "$(pwd):/src" composelint/compose-lint:0.27.0
```
...
```bash
docker run --rm -v "$(pwd):/src" composelint/compose-lint:0.27.0 docker-compose.prod.yml
```
