...
```bash
curl -fsSLO https://raw.githubusercontent.com/tmatens/compose-lint/v0.30.0/requirements.lock
pip install --require-hashes -r requirements.lock   # dependencies, hash-pinned
pip install --no-deps compose-lint==0.30.0          # the tool, version-pinned
```
...
```bash
docker run --rm -v "$(pwd):/src" composelint/compose-lint:0.30.0
```
...
```bash
docker run --rm -v "$(pwd):/src" composelint/compose-lint:0.30.0 docker-compose.prod.yml
```
...
jobs:
  compose-lint:
    ...
    steps:
      ...
      - uses: tmatens/compose-lint@adf00708734b772686337b69831c8e3912fa0ed4 # v0.30.0
        with:
