...
- **A `reason` without `enabled: false` warns.** Inside a rule block, `reason` is the justification that goes with a suppression. On its own it suppresses nothing — the rule stays on and still fails the build — so a lone `reason:` warns and names the rule id.
