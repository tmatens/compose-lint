...
Other values (e.g., `ipc: shareable`) are not flagged. The comparison is case-sensitive, as Docker's is: `pid: HOST` and `ipc: HOST` are refused by the daemon (`invalid PID mode: HOST`), so the container never starts sharing a namespace and there is nothing to flag.
