...
- **Evidence:** `_cl0007` (the rootfs is writable by default) and `_t7_volume_writable` (a named volume stays writable *under* `read_only`, which is why persistence through a volume is out of this rule's reach)
