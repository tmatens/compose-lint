...
- **Evidence:** `_cl0017` proves only that a shared bind propagation is flagged `shared:` in the container's mountinfo — it does not test either propagation leg. The directional claims are a separate manual `unshare -m` capture (recorded in the severity review): the host → container direction needs no capability, and the reverse is gated by `CAP_SYS_ADMIN` (CL-0024) and an unconfined AppArmor profile (CL-0009), both flagged separately
