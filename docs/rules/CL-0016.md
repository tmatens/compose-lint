...
- `device_cgroup_rules:` — express exactly which devices the container may access. compose-lint does not read this key yet, so a rule that opens the gate to a disk major (`b 8:* rwm`) is not flagged: keep it to the specific majors and minors the workload needs
