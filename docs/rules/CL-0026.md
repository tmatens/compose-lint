...
A memory value is read with the grammar Compose uses (go-units): a number,
optionally one space, then a scale letter `k`, `m`, `g`, `t` or `p` alone or
followed by `b` or `ib`, in any case, or a bare `b`. So `512m`, `1GiB`,
`262144KiB` and `1t` are all limits. A suffix Compose refuses (`512Mi`,
`1bb`) is not a limit and is flagged.
