...
- **Shipped:** CRITICAL
- **Evidence:** `_cl0001` in `scripts/validate_rule_premises.py`. A `:ro`-mounted socket is still a working API endpoint — a read-only bind completed a `GET /version`, so the mount flag does not narrow the grant
