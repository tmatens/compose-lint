...
A socket can also arrive as a **`secrets:` or `configs:` file**. Outside swarm,
`secrets: dsock: file: /var/run/docker.sock` is a read-only bind mount of that
host file at `/run/secrets/dsock` — the same mount `volumes:` would make, under
another key. Measured on Docker 29.7.2 / Compose 5.4.0: the container saw the
host inode, and `docker -H unix:///run/secrets/dsock version` was answered by
the daemon. Read-only changes nothing here, as above. The rule reads both
channels for a service that references the entry (short list or long
`source:`); a project-relative `file: ./secrets/…` — the pattern
[CL-0020](CL-0020.md) recommends — is not a host path and is not graded, nor is
an `external: true` or `environment:`-sourced entry.
...
| Technique | Tactic |
|---|---|
| [T1610 Deploy Container](https://attack.mitre.org/techniques/T1610/) | Execution |
| [T1609 Container Administration Command](https://attack.mitre.org/techniques/T1609/) | Execution |
| [T1611 Escape to Host](https://attack.mitre.org/techniques/T1611/) | Privilege Escalation |
| [T1612 Build Image on Host](https://attack.mitre.org/techniques/T1612/) | Stealth |
| [T1552.007 Unsecured Credentials: Container API](https://attack.mitre.org/techniques/T1552/007/) | Credential Access |
...
- [CL-0024](CL-0024.md) — `cap_add: [ALL]`, functionally similar in blast radius
- [CL-0025](CL-0025.md) — writable root-equivalent host mounts, the other no-technique route to host root
- [CL-0013](CL-0013.md) — sensitive host paths that are *not* control sockets
