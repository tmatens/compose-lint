...
2. **A mount of a directory that *contains* one** — `/run`, `/var/run`, `/run/containerd`, `/run/systemd`, or any path under them. This was the rule's blind spot: mounting `/run` hands over both daemon sockets without naming either, and it used to be reported as a generic sensitive-path finding one tier down (or, before #513, not at all).
