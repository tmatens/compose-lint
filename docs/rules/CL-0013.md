...
The same paths handed over as a **`secrets:` or `configs:` `file:`** are graded here too, always as read-only: outside swarm that entry is a read-only bind of the host file (measured — a write is refused even with `mode: 0666`), so `configs: shadow: file: /etc/shadow` is the read-only `/etc` disclosure above. A relative `file:` resolves against the compose file's directory exactly as a relative bind source does, so `file: ../../../../etc/shadow` is graded as the `/etc/shadow` it ships as. A project-relative `file: ./secrets/…`, the pattern [CL-0020](CL-0020.md) recommends, resolves inside the project and is not graded.
