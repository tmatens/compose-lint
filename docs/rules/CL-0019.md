...
- **Shipped:** MEDIUM
- **Evidence:** Not runtime-observable — compose-lint does not read the registry ([ADR-020](../adr/020-severity-scoping-and-overrides.md)). The premise is the registry API contract: a tag is a mutable pointer, and a digest is content-addressed, so a digest-pinned pull cannot silently resolve to different bytes

**References:**
- [Docker docs — pull an image by digest (immutable identifier)](https://docs.docker.com/reference/cli/docker/image/pull/#pull-an-image-by-digest-immutable-identifier)
- CIS Docker Benchmark 5.28 — Ensure that Docker commands always make use of the latest version of their image (its remediation is a pinning mandate)
- [OWASP Docker Security Rule #13](https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html#rule-13-enhance-supply-chain-security) — general supply-chain framing; note it contains no digest-specific guidance
...
| Technique | Tactic |
|---|---|
| [T1204.003 User Execution: Malicious Image](https://attack.mitre.org/techniques/T1204/003/) | Execution |
| [T1525 Implant Internal Image](https://attack.mitre.org/techniques/T1525/) | Persistence |
