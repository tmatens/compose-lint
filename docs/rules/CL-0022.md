...
- **Precondition:** Removes a mitigation — `exec`, `suid` and `dev` on a tmpfs grant no privilege. They remove three defaults Docker applies to every tmpfs
- **Impact:** Single container
- **Qualifier/modifier:** none
- **Derived:** Removes a mitigation × Single container = LOW
- **Shipped:** LOW
- **Evidence:** `_cl0022` and `_t22_exec_tmpfs` — a tmpfs is `noexec` by default and `:exec` removes it, re-proven against a live container on every CI run
