...
It refuses, and leaves the entry for you, when the service is built from a YAML anchor or merge key, when the list is written in flow style (`tmpfs: [/tmp:exec]`), when the entry carries `${VAR}` interpolation, or when the line does not show the whole value (a wrapped or block scalar). See [the fix contract](../fix.md).
