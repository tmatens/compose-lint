...
- Keys that name a **quantity about** the credential — a lifetime, size, limit or policy knob (lifetimes `TTL`, `TIMEOUT`, `EXPIRE`, `EXPIRY`, `EXPIRATION`, `VALIDITY`, `LIFETIME`, `MAX_AGE`/`MAXAGE`, `ROTATION`, `INTERVAL`, `RETENTION`, `DURATION`; time units `_SECONDS`/`_SECS`, `_MINUTES`/`_MINS`, `_HOURS`, `_DAYS`, `_MS`; sizes and policy knobs `MIN_LENGTH`/`MINLENGTH`, `MAX_LENGTH`/`MAXLENGTH`, `MIN_CHAR`/`MINCHAR`, `_LENGTH`, `_SIZE`, `_LIMIT`, `POLICY`, `REQUIREMENTS`, `_BONUS`, `_STRENGTH`, `_HISTORY`, `_ATTEMPTS`, `_RETRIES`, plural `TOKENS`; work factors `ROUNDS`, `ITERATIONS`, `_COST`; and `_PORT`) — **and** whose value is a bare quantity (`30`, `1.5`, `900s`, `30m`). `JWT_ACCESS_TOKEN_EXPIRE_MINUTES: 30` is a duration, not a token.
