...
Any service that does not end up with `no-new-privileges` set by its `security_opt:` list.

Docker applies the list in order, so the **last** entry naming `no-new-privileges` decides: `[no-new-privileges:true, no-new-privileges:false]` starts with the flag off. That is also what `extends:` produces when a child un-hardens its base, because Compose concatenates the two lists. The value is read the way Docker reads it, with Go's `strconv.ParseBool`: `1`, `t`, `T`, `true`, `True` and `TRUE` all set the flag; `0`, `f`, `F`, `false`, `False` and `FALSE` clear it; the bare `no-new-privileges` sets it.
