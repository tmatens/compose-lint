...
holds no other `/usr/lib` bind to shape a narrower match on. The disposition is
[ADR-033](../adr/033-library-tree-is-root-equivalent-by-containment.md), which
proposes `/lib/modules` by descent and `/usr/lib`, `/lib`, `/lib64` exactly.
